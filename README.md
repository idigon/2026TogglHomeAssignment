...
<details>
<summary>Two things you might not notice</summary>

Both are additive, appear only after the populated week, and touch nothing in
the core flow.

1. **A thirteenth entry that is not client work.** Late on Saturday, on its own
   project chip, already logged. It is seeded straight into the logs without
   entering the log order, so it raises no notification, feeds no benchmark and
   cannot move a single estimate. Open it.
2. **Ask Toggl answers once.** The sidebar bar, or Ctrl/Cmd+K, once you are past
   onboarding. It has exactly one thing to say, which is the same argument the
   rest of the prototype makes, said shorter.
